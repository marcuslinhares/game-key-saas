...
def main():
    ...
    # Leitura real de cobertura (Vitest coverage-summary.json)
    coverage_percent = 0
    if result["passed"] and with_coverage:
        summary_path = project_path / "coverage" / "coverage-summary.json"
        if summary_path.exists():
            try:
                data = json.loads(summary_path.read_text())
                # Pega a média total de linhas cobertas
                raw_pct = data.get("total", {}).get("lines", {}).get("pct", 0)
                try:
                    coverage_percent = float(raw_pct)
                    print(f"Coverage: {coverage_percent}%")
                except (ValueError, TypeError):
                    coverage_percent = 0
                    print(f"Warning: Invalid coverage percentage format: {raw_pct}")
            except Exception as e:
                print(f"Warning: Could not parse coverage summary: {e}")
        else:
            print("Warning: coverage-summary.json not found in coverage/ directory")

    threshold = 50 # Limite inicial mais baixo para o MVP
    if result["passed"]:
        if with_coverage and coverage_percent < threshold and result["tests_run"] > 0:
            print(f"[FAIL] Coverage {coverage_percent}% is below threshold ({threshold}%)")
            result["passed"] = False
        else:
            print(f"[PASS] All tests passed (Coverage: {coverage_percent}%)")
    else:
