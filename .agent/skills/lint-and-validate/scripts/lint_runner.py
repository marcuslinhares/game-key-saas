...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            if "lint" in scripts:
                result["linters"].append({"name": "npm lint", "cmd": ["npm", "run", "lint", "--", "--ignore-pattern", "coverage"]})
            elif "eslint" in deps:
                result["linters"].append({"name": "eslint", "cmd": ["npx", "eslint", ".", "--ignore-pattern", "coverage"]})
...
def run_linter(linter: dict, cwd: Path) -> dict:
    ...
    try:
        ...
        # Consider PASSED if exit code is 0 (even with warnings)
